...
interface DiagnosticInfo {
  ...
  stripeKeyType: 'test' | 'live' | 'missing' | 'placeholder';
...
export function ProdDiagnostics() {
  ...
  useEffect(() => {
    ...
    if (showDebug) {
      ...
      let stripeKeyType: 'test' | 'live' | 'missing' | 'placeholder' = 'missing';
      if (stripeKey) {
        if (stripeKey.includes('YOUR_LIVE_KEY') || stripeKey.includes('pk_live_YOUR')) {
          stripeKeyType = 'placeholder';
  ...
  const hasIssues = 
    ...
    info.stripeKeyType === 'missing' ||
    info.stripeKeyType === 'placeholder' ||
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gap: '4px' }}>
        ...
        <Row label="Stripe Key" value={
          info.stripeKeyType === 'test' ? 'Test Mode' :
          info.stripeKeyType === 'live' ? 'Live Mode' :
          info.stripeKeyType === 'placeholder' ? 'Placeholder!' :
          'Missing'
        } warn={info.stripeKeyType === 'missing' || info.stripeKeyType === 'placeholder'} />
